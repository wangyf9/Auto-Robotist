...
def compute_voxel_diff(parent_body, child_body):
    diffs = []
    for row in range(len(parent_body)):
        for col in range(len(parent_body[row])):
            parent_val = int(parent_body[row][col])
            child_val = int(child_body[row][col])
            if parent_val != child_val:
                diffs.append((row, col, parent_val, child_val))
    return diffs
